...
### 11. **Run RFC technical reviews (in parallel):**
   - **CRITICAL: Launch all RFC review agents in parallel**
   - Use a single message with multiple Task tool calls:

     **Task 1 - RFC Reviewer:**
     ```
     Use the rfc-reviewer agent to analyze the RFC at [path to RFC file]
     and evaluate its technical merit, problem-solution fit, assumptions, and evidence.
     Focus on technical soundness, not presentation.
     ```

     **Task 2 - Gemini RFC Reviewer:**
     ```
     Use the gemini-rfc-reviewer agent to analyze the RFC at [path to RFC file]
     and provide a technical review using Google's Gemini model. Evaluate logic,
     problem-solution fit, assumptions, evidence, and technical soundness.
     ```

     **Task 3+ - Domain Experts (if detected in RFC content):**

     Scan the RFC for domain-specific patterns (reference `commands/shared/domain-agent-registry.md`):

     | Domain Detected | Add Task |
     |-----------------|----------|
     | Flyte/Liftoff | `flyte-liftoff-expert` |
     | Luigi | `luigi-workflow-expert` |
     | Scio/Beam | `scio-pipeline-optimization-analyzer` |
     | Hendrix/ML | `hendrix-expert` |
     | Search/Indexing | `search-indexing-expert` |
     | Backend Infra | `backend-infrastructure-expert` |
     | RCS/Experimentation | `experimentation-expert` |

     **Prompt for domain experts:**
     ```
     Review the [DOMAIN] aspects of this RFC at [path to RFC file].
     Evaluate alignment with Spotify [DOMAIN] best practices,
     identify [DOMAIN]-specific issues, and suggest improvements.
     ```

   - All agents (standard + domain) review in parallel
   - Wait for all reviews to complete

### 12. **Synthesize and present technical review findings:**
   - Combine findings from all technical review agents:
     - Technical review 1 (rfc-reviewer)
     - Technical review 2 (gemini-rfc-reviewer)
     - Domain expert reviews (if spawned)

   - Present a consolidated summary:
     ```
     ## Technical Review Summary

     **RFC Reviewer Findings:**
     [Key findings and critical issues]

     **Gemini RFC Reviewer Findings:**
     [Key findings and critical issues]

     **Critical Issues (Both Reviews):**
     - [Issue 1]
     - [Issue 2]
     ...

     **Recommended Actions:**
     1. [Prioritized action from both reviews]
     2. [Prioritized action from both reviews]
     ...
     ```
...
### 17. **Validate Google Doc Formatting Visually:**
   - **CRITICAL: Use google-docs-visual-formatter subagent**
   - Invoke the subagent with Task tool:
     ```
     Use the google-docs-visual-formatter subagent to compare and fix formatting
     for the markdown file at [MARKDOWN_PATH] and Google Doc at [GOOGLE_DOC_URL].

     The subagent will:
     1. Capture screenshots of both the rendered markdown and Google Doc
     2. Detect formatting differences using multimodal LLM vision
     3. Apply all fixes in a single batch update to the Google Doc
     4. Validate with SSIM metric (iterate up to 3 times for SSIM ≥0.95)
     5. Clean up all temporary files
     ```
   - Pass the markdown file path (e.g., `rfcs/2025-11-13-rfc-video-intent-support.md`)
   - Pass the Google Doc URL from step 16
   - The subagent will execute a 6-phase automated workflow:
     - Phase 1: Capture screenshots
     - Phase 2: Quick pixel pre-screening (skip if <1% different)
     - Phase 3: LLM vision analysis of formatting differences
     - Phase 4: Apply formatting fixes via Google Docs API batch update
     - Phase 5: Validation & iteration (SSIM check, loop if needed)
     - Phase 6: Cleanup all temporary files from /tmp/
   - Review the subagent's output:
     - **Complete Success**: SSIM ≥0.95 - formatting matches rendered markdown
     - **Partial Success**: SSIM 0.85-0.95 after 3 iterations - minor differences remain
     - **Needs Review**: SSIM <0.85 - manual formatting may be required
   - If partial success or needs review, inform user of remaining differences
   - If complete success, confirm to user that visual formatting validation passed
