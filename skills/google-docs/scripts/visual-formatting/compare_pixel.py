...
def compare_images_with_odiff(
    image1_path, image2_path, diff_output_path, threshold=0.1
):
    """
    ...
    try:
        ...
        if percent_match:
            ...
            return {"diff_percent": 0.0, "diff_pixels": 0, "status": "identical"}

    except FileNotFoundError:
        print(
            "Error: odiff not found. Install with: npm install -g odiff-bin",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Sensitivity threshold (default: 0.1)",
    )

    args = parser.parse_args()

    result = compare_images_with_odiff(
        args.image1, args.image2, args.output, args.threshold
    )
    ...
    if result["diff_percent"] < 1.0:
        print(
            f"\nImages are very similar ({result['diff_percent']}% different)",
            file=sys.stderr,
        )
        sys.exit(0)
    else:
        print(
            f"\nImages have noticeable differences ({result['diff_percent']}% different)",
            file=sys.stderr,
        )
        sys.exit(1)
